...
class IndexDataTest : BaseTest() {
    ...
    @Test
    fun `indexData should return success when data is indexed successfully`() =
        runTest {
            ...
            val actual = subject.indexData(dataSource)
            assert(actual.isSuccess)
        }

    @Test
    fun `indexData should return failure when data is not indexed successfully`() =
        runTest {
            val dataSource = SQLDataSource(
                identifier = RandomPrimitivesFactory.genRandomString(),
                databaseType = SQLDataSource.DatabaseType.H2,
